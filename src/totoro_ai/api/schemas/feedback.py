...
class FeedbackRequest(BaseModel):
    ...
    place_id: str = Field(
        ..., description="The place the user acted on (primary or alternative)"
    )
    signal: Literal["accepted", "rejected"] = Field(
        ..., description="Whether user accepted or rejected"
    )
...
class FeedbackResponse(BaseModel):
    ...
    status: str = Field(
        "received", description="Feedback received and queued for processing"
    )
