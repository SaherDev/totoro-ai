...
**Superseded by:** Existing downstream typo tolerance mechanisms (LLM intent parser + Google Places fuzzy matching)\
...
**Decision:** A `SpellCorrectorProtocol` (@runtime_checkable Protocol) defines the contract: `correct(text: str, language: str = "en") -> str`. Implementations wrap different libraries: `SymSpellCorrector` (default, wraps symspellpy), future variants for other spell correction libraries. The active corrector is loaded at FastAPI startup via `get_spell_corrector()` factory reading `config/app.yaml` under `spell_correction.provider` (e.g., `symspell`). All three endpoint handlers (extract-place, consult, recall) call `spell_corrector.correct(query)` as the first step before parsing, embedding, or LLM calls. Language defaults to "en" in this iteration; future iterations can read user locale from database. Raw input travels untouched from Next.js through NestJS to FastAPI. FastAPI corrects it silently. The corrected text is what gets embedded, stored in places.place_name, and used in LLM queries. The LLM system prompt for intent parsing also includes an explicit instruction to interpret input regardless of spelling as a second layer of tolerance. Google Places API fuzzy matching acts as a third layer for place name typos during validation.\
**Consequences (Original):** Implementation complete as of 2026-03-31. Module `src/totoro_ai/core/spell_correction/` contains `SpellCorrectorProtocol` (@runtime_checkable Protocol in base.py) and `SymSpellCorrector` (symspell.py). Factory `get_spell_corrector()` in `src/totoro_ai/providers/spell_correction.py` reads `get_config().spell_correction.provider` and uses @functools.lru_cache(maxsize=1) to avoid per-request 29MB dictionary reloads. Wired into `ExtractionService`, `ConsultService`, and `RecallService` via `src/totoro_ai/api/deps.py`. SymSpellCorrector preserves URLs (never corrected) and implements error fallback (any exception returns original text). Test coverage: 6 unit tests + 3 factory tests, all 93 project tests pass.\
**Supersession Rationale (2026-03-31):** During implementation testing, spell correction proved actively harmful for domain-specific terms. Example: "ramin" (common misspelling of "ramen") was "corrected" to "rain" because generic English dictionaries don't prioritize food/place names. The LLM intent parser already includes explicit instruction "interpret input regardless of spelling" (second layer of defense). Google Places API provides fuzzy matching on place names as a third layer. These two mechanisms alone provide sufficient typo tolerance without a dedicated spell correction layer. Dedicated spell correction adds complexity and risk of false corrections on domain-specific vocabulary. **Recommendation:** Do not merge 007-spell-correction branch. Rely on existing intent parser resilience and Google Places fuzzy matching for typo handling. Close feature branch.
