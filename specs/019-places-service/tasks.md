...
- [ ] T053 [US3] **Full rewrite of `src/totoro_ai/core/intent/intent_parser.py` from scratch** around the new schema. The current code predates `PlaceObject`; it invented its own "place" vocabulary and every downstream service translated from it. Delete both `_IntentLLMOutput` AND the old `ParsedIntent` entirely. Replace with a single new `ParsedIntent` whose field names match `PlaceObject` / `PlaceAttributes` / `RecallFilters` exactly — no translation layer anywhere downstream.
  ...
  **System prompt rewrite**: every field reference in the prompt must use the exact vocabulary from `PlaceAttributes` — no invented synonyms, no remapping. Include at least these five examples inline in the prompt:
  ...
  → place_type: "food_and_drink", subcategory: "cafe", ambiance: "cozy",
    good_for: ["solo"], enriched_query: "cozy cafe solo"

  Query: "late night drinks"
  → place_type: "food_and_drink", subcategory: "bar", good_for: ["late-night"],
    enriched_query: "late night bar"

  Query: "halal food nearby"
  → place_type: "food_and_drink", dietary: ["halal"], radius_m: 500,
...
- [X] T055 [P] [US3] Update any existing test in `tests/core/consult/`, `tests/core/taste/`, `tests/core/ranking/` that constructs `Place` ORM rows or references legacy fields. Replace with `PlaceObject` factories. Depends on T049, T050, T051, T052.
